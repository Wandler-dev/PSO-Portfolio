...
Web 可视化界面的用户可见标签统一采用中文表达，专有指标采用“中文 + 英文括号”的形式，例如“期望收益率（Expected Return）”“波动率（Volatility）”“夏普比率（Sharpe Ratio）”。前端内部仍保持 `expected_return`、`volatility`、`sharpe_ratio` 等 API 字段不变。

入选策略表格适配深色 Dashboard 样式：表头、行背景、边框、悬停状态和文字颜色均使用深色界面下可读的配色；表格列名显示为“候选策略”和“权重”，权重以百分比展示。表格区域采用紧凑布局，行数较多时滚动显示，避免在结果较少时产生过多留白。
