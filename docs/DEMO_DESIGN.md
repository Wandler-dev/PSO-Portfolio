...
顶部数据集信息不直接展示后端英文 `source_notes`，而是在前端显示统一的中文数据说明：UCI 数据集不是原始股票价格序列，每个 ID 被解释为候选选股权重策略，PSO 在候选策略之间优化资金分配权重。

参数控制面板中的三个预设按钮必须保持相同宽度、高度、间距和字体大小，激活态只改变颜色，不改变尺寸。无风险利率在三个预设中保持 `0.0`，前端通过辅助说明标注其为课程实验固定值。

单次优化结果区域按“收敛曲线 + Top 10 权重图、风险-收益散点图、入选策略表 + 当前组合说明”的顺序组织。预设对比区域独立放置在单次优化结果之后，保证图表边界对齐、信息密度适中、页面更紧凑。
